...
$pID = $_GET['pID'];
$query = 
    "SELECT * FROM bnc353_2.Person as Person, bnc353_2.NotRegistered as nonReg
    WHERE Person.pID = '$pID' AND Person.pID = nonReg.pID;";
...
if(isset($_POST["firstName"]) 
    ...
    if($conn->query($query) == true){
        ...
        $query = "UPDATE bnc353_2.NotRegistered SET 
                                    passNum = '$passNum'
                                    WHERE pID = '$pID';";
        if($conn->query($query) == true){
            ...
            if (headers_sent()) {
                die("Redirect failed. Please click on this link: <a href='./edit-non-registered.php?pID= .$_POST[pID]''>Retry</a>");
            }
            else{
                exit(header("Location: ./edit-non-registered.php?pID=" .$_POST["pID"]));
            }
        }
    }
}
        ...
        <div class="w3-content">
            <div>
                ...
                <div class="container px-6 py-2">
                    ...
                    <form action="./edit-non-registered.php?pID=<?= $person["pID"] ?>" method="post" class="flex flex-col border border-gray-50 p-4 border-separate flex flex-col items-center w-full">
                        ...
                        <input type="number" name="passNum" id="passNum" class="border-2 border-gray-100 rounded-md px-2 py-2" value="<?= $person["passNum"]?>"> <br>
