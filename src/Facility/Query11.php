...
WHERE facility = '$facilityName' AND date BETWEEN '$from' AND '$to') fullSchedule
        ...
        <div class="w3-content">
            <div>
				...
				<div class="flex flex-row px-6 py-4 border-b border-gray-100 justify-between items-center">
					...
					<div class="flex flex-row px-6 py-4 border-b border-gray-100 justify-between items-center">
						<h1 class="text-2xl m-0">Booking Schedule from <?= date("F jS, Y", strtotime($from)) ?> to <?= date("F jS, Y", strtotime($to)) ?></h1>
					</div>
